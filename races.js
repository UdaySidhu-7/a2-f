...
async function loadSeasonData(season) {
  ...
  if (localStorage.getItem(`season_${season}_races`)) {
      console.log(`Data for season ${season} found in localStorage.`);
  } else {
      console.log(`Fetching data for season ${season}...`);

      // Fetch races
      const racesResponse = await fetch(`${API_DOMAIN}/races.php?season=${season}`);
      const races = await racesResponse.json();
      localStorage.setItem(`season_${season}_races`, JSON.stringify(races));

      // Fetch all results
      const resultsResponse = await fetch(`${API_DOMAIN}/results.php?season=${season}`);
      const results = await resultsResponse.json();
      localStorage.setItem(`season_${season}_results`, JSON.stringify(results));

      // Fetch all qualifying
      const qualifyingResponse = await fetch(`${API_DOMAIN}/qualifying.php?season=${season}`);
      const qualifying = await qualifyingResponse.json();
      localStorage.setItem(`season_${season}_qualifying`, JSON.stringify(qualifying));

      console.log("All season data fetched and cached.");
  }
  loadRaces();
}
...
function showDriverModal(driverId,driverRef, seasonYear, points) {
  ...
  const url = `https://www.randyconnolly.com/funwebdev/3rd/api/f1/driverResults.php?driver=${driverRef}&season=${seasonYear}`;
...
.then((driver) => {
  document.getElementById('driverName').innerText = `${driver.forename} ${driver.surname}`;
  ...
  document.getElementById('driverURL').href = driver.url;
  document.getElementById('driverURL').innerText = "Wikipedia";

  // Optionally, load an image using driver.code or driverRef
  const imageUrl = `https://placehold.co/600x400?text=${encodeURIComponent(driver.forename +" "+ driver.surname || "Circuit")}`;
